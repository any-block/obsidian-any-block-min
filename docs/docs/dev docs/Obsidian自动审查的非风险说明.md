...
- 要么重构原代码对于 Core 模块的引用路径，变为 npm 的形式。这可能会修改非常多的代码
- 要么利用 copyfiles 等类似工具，添加 'prebuild'，在编译前将该模块从 node_module 中移回原位置，这样不需要改动路径
- 或者修改 tsconfig 中的别名，使原来对 Core 模块的引用通过 '别名' 转到 node_module 中
